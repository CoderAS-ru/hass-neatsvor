...
class NeatsvorEnumSelect(CoordinatorEntity, SelectEntity):
    ...
    def _get_current_value(self) -> Optional[str]:
        ...
        # Используем правильные коды DP из DPManager
        # CODE_FAN = 'fan', CODE_WATER_TANK = 'water_tank'
        key_map = {}
        for code in ['fan', 'water_tank', 'clean_mode']:
            dp = self.coordinator.vacuum.dp_manager.get_by_code(code)
            if dp:
                key_map[dp.id] = code
        
        # Маппинг кодов на ключи в данных координатора
        value_key_map = {
            'fan': 'fan_speed',
        ...
        code = key_map.get(self._dp_id)
        if code:
            data_key = value_key_map.get(code)
            if data_key:
