...
class AsyncMapVisualizer:
    ...
    async def render_static_map(self, map_data: Dict[str, Any],
        ...
        Args:
            map_data: Decoded map data
            title: Title for the map (used in filename)
            map_type: Type of map ('realtime', 'history', 'cloud')

        Returns:
            Path to saved PNG file or None on error
        """
        _LOGGER.info("render_static_map: title=%s, map_type=%s", title, map_type)
        _LOGGER.info("map_data type: %s", type(map_data))
        ...
        try:
            ...
            # Synchronous rendering in executor
            _LOGGER.info("Starting synchronous rendering in executor...")
            ...
            _LOGGER.info("render_static_map returned: %s", filename)

            # Check if file exists
            if filename and Path(filename).exists():
                file_size = Path(filename).stat().st_size
                _LOGGER.info("File created: %s (%s bytes)", filename, file_size)
            else:
                _LOGGER.warning("File not found or not created: %s", filename)
