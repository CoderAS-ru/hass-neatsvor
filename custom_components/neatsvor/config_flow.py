    ...
    async def async_step_user(
        ...
        if user_input is not None:
            try:
                ...
                # Try to connect in test mode (без фоновых задач)
                _LOGGER.info("Connecting with app %s, data center %s...", 
                            app_config["name"], data_center["country_name"])
                
                # Создаем вакуум в тестовом режиме
                vacuum = NeatsvorVacuum(config, app_type=app_type)
                
                # Устанавливаем флаг тестового режима, чтобы отключить фоновые задачи
                vacuum._test_mode = True
                ...
                try:
                    ...
                    # Всегда отключаемся после теста
